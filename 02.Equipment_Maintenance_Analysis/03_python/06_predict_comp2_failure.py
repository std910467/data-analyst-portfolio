...
df_daily[[f"{col}_roll3_mean" for col in sensor_cols]] = df_daily.groupby("machineID")[sensor_cols].transform(lambda x: x.rolling(window=3, min_periods=1).mean())
df_daily[[f"{col}_roll3_sum"  for col in count_cols]]  = df_daily.groupby("machineID")[count_cols].transform(lambda x: x.rolling(window=3, min_periods=1).sum())
...
# 設定預警條件
cond_rotate = (df_daily["d_rotate_roll3_mean"]-df_daily["d_rotate_roll3_lag3"])/df_daily["d_rotate_roll3_lag3"] < -0.05
cond_error2 = df_daily["error2_times_roll3_sum"]-df_daily["error2_times_roll3_lag3"] > 0.5
cond_error3 = df_daily["error3_times_roll3_sum"]-df_daily["error3_times_roll3_lag3"] > 0.5
...
df_daily["comp2_failure"].sum()
df_daily["warning_signal"].sum()


# 未來2天內只要有任何一天故障就算 True
df_daily["comp2_fail_in_next_2days"] = (df_daily["comp2_fail_next1"] == 1) | (
    df_daily["comp2_fail_next2"] == 1
)
#  前2天內只要有任何一天警告就算True
...
failures = df_daily[df_daily["comp2_failure"] == 1]
